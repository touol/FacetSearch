...
class facet{
	...
	public function __construct() {
		...
		$this->client = (new \OpenSearch\ClientBuilder())
			...
			->setBasicAuthentication('admin', 'ijUaD16XHB') // For testing only. Don't store credentials in code.
	...
	private function check_access($data = []){
		if(file_exists(__DIR__ . '/secret/secret_main.json')){
			$access = json_decode(file_get_contents(__DIR__ . '/secret/secret_main.json'),1);
			if(isset($access[$data['index']]) and $access[$data['index']] == $data['api_key']) return true;
		}
		if(file_exists(__DIR__ . '/secret/secret.json')){
			$access = json_decode(file_get_contents(__DIR__ . '/secret/secret.json'),1);
			if(isset($access[$data['index']]) and $access[$data['index']] == $data['api_key']) return true;
		}
		return false;
	}
	...
	public function run(){
		$access = $this->check_access($_POST);
		if(!$access) return $this->error('Access denied!'.$access);
