FacetSearch.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper facetsearch-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('facetsearch-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('facetsearch-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('facetsearch-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('facetsearch-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    FacetSearch.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(FacetSearch.grid.Default, MODx.grid.Grid, {
    ...
    getSearchField: function (width) {
        return {
            xtype: 'facetsearch-field-search',
            width: width || 250,
            listeners: {
                search: {
                    fn: function (field) {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = FacetSearch.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('facetsearch-grid-default', FacetSearch.grid.Default);